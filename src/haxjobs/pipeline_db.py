...
def action_classify_roles(argv: list[str] | None = None):
    ...
    from haxjobs.db.role_classification import classify_existing_jobs
    limit = None
...
def action_status():
    """Print DB stats."""
    from haxjobs.db.stats import get_stats
    s = get_stats()
...
def action_pending():
    """List pending jobs."""
    from haxjobs.db.jobs import get_pending_jobs
...
def action_scrape_greenhouse(argv: list[str]):
    from haxjobs.discovery.scrapers.greenhouse import main as greenhouse_main
    raise SystemExit(greenhouse_main(argv))


def action_scrape_ashby(argv: list[str]):
    from haxjobs.discovery.scrapers.ashby import main as ashby_main
    raise SystemExit(ashby_main(argv))


def action_scrape_lever(argv: list[str]):
    from haxjobs.discovery.scrapers.lever import main as lever_main
    raise SystemExit(lever_main(argv))


def action_scrape_all():
    """Run every configured discovery scraper."""
    from haxjobs.discovery.scrapers.orchestrator import run_all_scrapers, summarize_results
    from haxjobs.db.schema import init as _init
    _init()
...
def action_run_full():
    ...
    print("\n─ Evaluation ─")
    from haxjobs.evaluate.run import evaluate_from_db
    count = 0
    while True:
        ok = evaluate_from_db()
        if not ok:
