...
from haxjobs_api.features.tasks.repository import create_hermes_task, list_hermes_tasks
...
@router.get("", response_model=list[HermesTaskRead])
def read_hermes_tasks(session: Session = Depends(get_db_session)):
    return list_hermes_tasks(session)
