...
After implementation, collect every deliverable artifact (never code) into
**one labelled folder** so the reviewer does not hunt across the repo:

```text
deliverables/001-stage0/
├── plan.md                              ← copy of this plan
├── report.md                            ← implementation report
├── job-49.json                          ← job fixture
├── job-328.json                         ← job fixture
├── rubric.md                            ← human review rubric
├── 003-stage0-observed-job-review.drawio
├── 003-stage0-observed-job-review.png
└── README.md                            ← one-paragraph summary + links
```

Every path in the folder is self-contained. Copy sources in — do not
symlink, because the folder may be archived or shared independently.

The plan source, test fixtures, and `src/` code stay where they are.
This folder is for the reviewer, not the runtime.
